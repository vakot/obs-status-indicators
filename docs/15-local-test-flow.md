...
The overlay is anchored to the primary display's top-left origin `(0, 0)`. Each active state is a 48x48 black square with an 8-pixel icon inset; multiple states stack downward with an 8-pixel gap. Recording, pause, Replay Buffer, microphone, and saving use distinct Lucide action glyphs, and a muted microphone uses the Lucide `mic-off` glyph.
