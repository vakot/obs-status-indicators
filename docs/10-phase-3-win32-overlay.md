...
The overlay uses `WS_POPUP` with `WS_EX_LAYERED`, `WS_EX_TRANSPARENT`, `WS_EX_NOACTIVATE`, and `WS_EX_TOOLWINDOW`. It reports `HTTRANSPARENT` for `WM_NCHITTEST` and `MA_NOACTIVATE` for `WM_MOUSEACTIVATE`, so it has no intended input/focus ownership. Each active indicator is a 64x64 opaque black square with an 8-pixel icon inset; active squares are stacked vertically with an 8-pixel gap and positioned 8 pixels from the primary display's bottom-left corner.
