...
The overlay uses `WS_POPUP` with `WS_EX_LAYERED`, `WS_EX_TRANSPARENT`, `WS_EX_NOACTIVATE`, and `WS_EX_TOOLWINDOW`. It reports `HTTRANSPARENT` for `WM_NCHITTEST` and `MA_NOACTIVATE` for `WM_MOUSEACTIVATE`, so it has no intended input/focus ownership. Each active indicator is a 48x48 black square at 50% opacity with an 8-pixel icon inset; active squares are stacked vertically with an 8-pixel transparent gap and positioned at the primary display's exact top-left origin `(0, 0)`.
