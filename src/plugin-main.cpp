...
static void state_changed(const IndicatorState &state, void *context)
{
	...
	obs_log(LOG_INFO, "state snapshot: recording=%d paused=%d replay=%d mic=%d muted=%d camera=%d saving=%d",
		state.recording, state.recordingPaused, state.replayBuffer, state.microphoneAvailable,
		state.microphoneMuted, state.cameraAvailable, state.saving);
...
bool obs_module_load(void)
{
	...
	obs_log(LOG_INFO, "overlay settings loaded: origin=%d orientation=%d offset=%d gap=%d size=%d opacity=%d background=0x%08x icon=0x%08x",
		static_cast<int>(overlay_settings.origin), static_cast<int>(overlay_settings.orientation),
		overlay_settings.offset, overlay_settings.gap, overlay_settings.indicator_size,
		overlay_settings.opacity,
